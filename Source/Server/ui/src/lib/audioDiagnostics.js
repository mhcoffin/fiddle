...
export function diagnosticReport(history, marks) {
    return JSON.stringify({
        description: "Fiddle audio diagnostics. Load is callback wall time / audio block duration, not whole-machine CPU. Plugin timings include waiting inside plugins. Per-plugin windows are independent: do not sum their peaks. Counts are cumulative; native counts may reset on plugin recreation. Sample times are UI receipt times, not exact glitch times.",
