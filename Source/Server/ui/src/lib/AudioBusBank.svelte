        ...
        <div class="channels">
            {#each buses as bus (bus.id)}
                ...
                <article class="channel" class:suppressed={bus.muted || (anySoloed && !bus.soloed)}>
                    <div class="identity" title={bus.name}>
...
<style>
    ...
    .header { min-height: 38px; padding: 5px 8px 5px 11px; box-sizing: border-box; display: flex; align-items: center; justify-content: space-between; gap: 8px; flex-shrink: 0; border-bottom: 2px solid #22d3ee; background: #164e63; }
    .header-toggle { min-width: 0; min-height: 27px; padding: 3px 5px; display: flex; align-items: center; gap: 7px; flex: 1; border: 0; border-radius: 4px; background: transparent; color: #cffafe; font: inherit; cursor: pointer; text-align: left; }
    .header-toggle:hover { background: rgba(8,47,73,.5); }
    .header-toggle strong { font-size: .76rem; white-space: nowrap; }
    ...
    .identity { min-width: 0; height: 38px; display: flex; flex-direction: column; justify-content: center; gap: 2px; text-align: center; }
